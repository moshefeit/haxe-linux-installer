...
if [ "$1" = "y" -o "$1" = "-y" ]; then
	...
	echo "Pasang Haxe $HAXE_VERSION dan Neko $NEKO_VERSION? (y/n) y"
	RESP=y
	
else
	
	read -p "Pasang Haxe $HAXE_VERSION dan Neko $NEKO_VERSION? (y/n) " RESP
...
if [ $RESP = "y" ]; then
	...
	if [ `uname -m` = "x86_64" ]; then
		...
		echo ""
		echo "---------------------------------------"
		echo "    Memasang Neko $NEKO_VERSION"
		echo "---------------------------------------"
